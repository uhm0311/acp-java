...
$cmd = "ssh $t_ip 'cd $dir && ./persistence/run.memcached.persistence.bash $t_port default_engine_sync_logging.conf' &";
system($cmd);
sleep 2;
...
foreach $script (@script_list) {
    ...
    $result_filename = "tmp-persistence-sync-performance-summary.txt";
