...
public class map_bulk_ins implements client_profile {
  ...
  public boolean do_map_test(client cli) throws Exception {
    ...
    for (int i = 0; i < loop_cnt; i++) {
      ...
      CollectionFuture<Boolean> fb =
          cli.next_ac.asyncMopCreate(key_list.get(i), vtype, attr);
    ...
    // repeat 1000
    for (long mkey = base; mkey < base + 1000; mkey++) {
      if (!cli.before_request())
        return false;
      ...
      Future<Map<String, CollectionOperationStatus>> fbs =
          cli.next_ac.asyncMopInsertBulk(key_list, input_mkey, val, new CollectionAttributes());
